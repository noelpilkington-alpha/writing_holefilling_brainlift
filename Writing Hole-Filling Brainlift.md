...
**Why Follow:** Datchuk's fluency research provides the evidence base for why full course enrollment is the default for G3-5 test-out failures. His work demonstrates that writing fluency requires varied, repeated practice to achieve automaticity. A student who passes a test through general ability has not necessarily automated the underlying skills. The G3-5 courses (6.5-7.8 hours) are short enough that full enrollment functions as a fluency-building investment. Skills the student has already mastered will be completed quickly (~5 minutes instead of ~15 minutes per lesson), serving as spaced review while building the automaticity that surface-level hole-filling alone cannot provide. However, students whose diagnostic profile shows no fundamental conceptual gaps have already demonstrated the automaticity Datchuk describes — for these students, targeted hole-filling with complete scaffolds is more appropriate (see Truth 3).
...
**Spiky POV Truth 3: Full enrollment is the default for G3-5 test-out failures. The diagnostic must earn the right to override it.** *(Updated v1.4)*
For G3-5 test-out failures (below 90%), full enrollment is the default because the courses are only 6.5-7.8 hours, untested scaffold skills (SPO, outlining, elaboration) are prerequisites for G6-8 essay success, and mastered skills fly by in ~5 minutes functioning as spaced review. However, the default can be overridden to targeted hole-filling when — and only when — the agent can demonstrate ALL of the following from the student's actual responses: (1) **No fundamental conceptual gaps** — every sentence-level error shows the student understands the concept but executes imperfectly (e.g., comma splice when attempting an appositive vs. not knowing what an appositive is; using "and" instead of "so" vs. writing a fragment to fix a fragment), (2) **Q11 paragraph score ≥ 16/20** — functional paragraph competence is demonstrated, and (3) **A complete targeted plan exists** — including all prerequisite chains and complete scaffolds for every identified gap, not just the surface-level AI recommendations. If ANY criterion fails, full enrollment stands. The score alone does not determine the intervention — a student at 84% with a fundamental fragment gap needs full enrollment just as much as a student at 80%, while a student at 82% whose errors are all execution-level may qualify for hole-filling. The diagnostic profile, not the percentage, is what matters.
...
**Spiky POV Truth 6: In G6-8, the essay IS the test. MCQs are diagnostic, not decisive.** *(Updated v1.4)*
The essay is worth 20 out of 30 points (66.7%). A student scoring 80%+ (24+/30) has necessarily performed well on the essay, even if they got every MCQ wrong (10/30 maximum from MCQs alone means they still needed 14+/20 on the essay). This asymmetry means the 80-89% tier demonstrates genuine essay competence under test conditions. However, MCQ performance is not noise — it carries real diagnostic value. Sentence-level weaknesses revealed by MCQs (fragments, appositives, conjunctions) may indicate skills that are not yet automatic, consuming working memory that should be available for essay planning and organization. The agent must use MCQ errors diagnostically to identify targeted S+P gaps, while recognizing that the essay score determines the action tier. Below 80%, essay performance is likely weak, and the full course (all S+P + all 5 essays) is needed.
...
**Spiky POV Myth 2: We don't believe that surface-level hole-filling alone can prepare a G3-5 student for the next grade.** *(Updated v1.4)*
When a student passes G4 at 90%+ and then scores 75% on the G5 test-out, the temptation is to hole-fill only the tested gaps. But the G5 test doesn't test every skill in the G5 course. Activities like Topic Brainstorm, SPO, Turn Outline into Draft, and Elaborate on Paragraphs are not directly tested but build the cognitive architecture for tested skills. And the G5-to-G6 transition introduces essays: a student entering G6 without solid paragraph planning skills will struggle with the essay scaffold. For students with fundamental conceptual gaps, full enrollment ensures every building block is in place. For students who meet the diagnostic gate criteria (Truth 3), targeted hole-filling with complete scaffolds — including untested prerequisite skills identified through response analysis — can provide equivalent preparation without redundant practice on mastered skills. The key distinction is between surface-level hole-filling (just the missed questions) and diagnostic hole-filling (root causes, prerequisites, and complete scaffolds).
...
**Spiky POV Myth 4: We don't believe that conventions errors (spelling, punctuation) indicate a skill gap requiring lesson assignment.** *(Updated v1.4)*
When a student demonstrates the correct skill but makes a mechanical error (e.g., writes a correct appositive but omits one comma), the root cause is conventions, not the skill itself. Assigning the skill activity would waste time. The agent must distinguish between skill deficits (assign the skill) and conventions errors (no assignment needed or assign a different, mechanics-focused activity). This distinction alone can cut unnecessary assignments by 20-30%. **Exception:** A *pattern* of the same conventions error across multiple questions may signal incomplete conceptual understanding rather than carelessness. A student who omits the second appositive comma once is making a conventions error. A student who omits it every time they attempt an appositive may have an incomplete mental model of appositives as parenthetical insertions. The agent should treat isolated conventions errors as noise and repeated conventions patterns as potential skill signals requiring closer examination.
...
**Spiky POV Truth 10: An incomplete scaffold is not remediation — it's a false receipt that the system cashed a check it never wrote.** *(Added v1.2, updated v1.4)*
When a skill requires a multi-step instructional scaffold (e.g., multi-section synthesis for G3-5), the COMPLETE sequence must be assigned. Assigning only the first step (e.g., "Identify Topic Sentences" alone) creates false coverage: the system records the intervention as delivered, the student builds a partial schema that cannot yet produce the target skill, and the core deficit remains unaddressed. The student retests, fails with the same error pattern, and the system has fewer remaining tools to deploy. The partial schema isn't harmful in itself — some learning occurred — but the false coverage is dangerous because it masks a gap that still needs closing. The complete scaffold is the minimum viable intervention.
...
- Insight 2: G3-5 course brevity (6.5-7.8h) means the time cost of full enrollment vs. hole-filling is only 3-5 additional hours. This makes full enrollment the default strategy for G3-5 test-out failures, with a diagnostic override to hole-filling permitted only when the agent can demonstrate no fundamental conceptual gaps, Q11 ≥ 16/20, and a complete targeted plan (see Truth 3). *(Updated v1.4)*
...
**Summary:** When a student passes their current grade (90%+) and then scores below 90% on the next grade's test, the agent must decide between targeted hole-filling and full enrollment. The decision depends on the grade band, the score tier, and — for G3-5 — the diagnostic profile. G3-5 defaults to full enrollment unless the diagnostic gate criteria are met (no fundamental conceptual gaps, Q11 ≥ 16/20, complete targeted plan exists — see Truth 3). G6-8 is tiered: 80-89% gets targeted S+P hole-filling plus all 5 essays; below 80% gets full enrollment (all S+P + all 5 essays). *(Updated v1.4)*

**Sources:** *(DOK 2)*

- **Datchuk's Fluency Research**
   - Summary: Datchuk's research on sentence-level fluency provides the evidence base for defaulting to full enrollment in G3-5 test-out failures. Writing fluency requires varied, repeated practice for automaticity. A student who passes a test through general ability has demonstrated competence but not necessarily fluency. The short G3-5 courses (6.5-7.8h) serve as fluency-building investments where mastered skills complete quickly and function as spaced review. However, for students whose diagnostic profile shows no fundamental conceptual gaps and strong Q11 performance, the fluency argument weakens: these students have already automated the core skills, and forcing them through 25+ mastered lessons provides diminishing returns while potentially undermining motivation (see Yeager, Expert 4).
   ...
   - Insights: The "mastered skills fly by quickly" insight supports full enrollment as the default. But case study analysis reveals that students with no fundamental gaps and Q11 ≥ 16/20 have already demonstrated the fluency Datchuk describes — their errors are execution-level, not automaticity-level. For these students, targeted hole-filling with complete scaffolds is more pedagogically sound than redundant practice. *(Updated v1.4)*
...
- **Math Academy's Encompassing Principle**
   ...
   - Insights: The encompassing principle is the primary argument for full enrollment as the default in G3-5 test-out failures. The untested scaffold activities aren't "extra": they're the structural foundation that makes tested skills durable and transferable. The diagnostic gate (Truth 3) respects this principle by requiring the agent to positively demonstrate that no fundamental gaps exist before overriding to hole-filling — the burden of proof is on the diagnostic, not the default. *(Updated v1.4)*
...
- **Bjork's Desirable Difficulties and the Transfer Gap**
   ...
   - Key facts: Desirable difficulties enhance long-term retention ONLY after competence is established. Spacing, interleaving, and reduced feedback help learners who already have schemas. For novices, these same conditions impair learning. The 80% threshold operationalizes this principle for G6-8, where the essay weight (66.7%) makes it a natural competence boundary.
   - Link: Bjork, "Making things hard on yourself, but in a good way" (2011)
   - Insights: The 80% threshold represents the competence boundary below which desirable difficulties become UNdesirable in G6-8. It is justified by the test architecture (essay weight makes 80%+ a reliable indicator of essay competence) and validated by Bjork's principle, though Bjork's research does not specify a numeric cutoff. Students below 80% need more support, not more challenge. *(Updated v1.4)*

**Insights on Category 4:** *(DOK 3)*
- Insight 1: The G5-to-G6 transition is the most critical in the system. G6 introduces essays (10.4 additional hours). A student entering G6 without solid paragraph planning skills will struggle with the essay scaffold. This is why the diagnostic gate should be applied conservatively for G5 test-out failures — the paragraph scaffold skills (SPO, Turn Outline into Draft, Elaborate) are direct prerequisites for G6 essay success, and the agent should include them in any targeted hole-filling plan when Q11 shows synthesis or organization weaknesses. *(Updated v1.4)*
- Insight 2: The 80-89% G6-8 tier is the only scenario where targeted S+P hole-filling (rather than full S+P) is assigned by default based on score alone. G3-5 targeted hole-filling is permitted only through the diagnostic gate (Truth 3), which requires positive evidence of no fundamental gaps. This asymmetry reflects the different course sizes and the different evidence available: G6-8 MCQs are recognition-level and the essay score directly demonstrates production-level competence, while G3-5 constructed-response questions provide richer diagnostic information about conceptual understanding. *(Updated v1.4)*
- Insight 3: Both G6-8 tiers (80-89% and below 80%) assign all 5 essay slots. Any previously completed slots carry over as "complete," so the student only does remaining unused slots. The difference between tiers is S+P assignment: 80-89% gets targeted gaps only; below 80% gets all S+P activities.
- Insight 4: **The score does not determine the error type.** *(Added v1.4)* Case study analysis of 8 G3-5 test-out students (80-87%) revealed that a student at 84% can have the same fundamental fragment gap as a student at 80%, while a student at 82% can have no fundamental gaps at all. The diagnostic profile — specifically, whether errors reflect conceptual misunderstanding or execution-level imprecision — is the determining factor for the enrollment-vs-hole-filling decision, not the percentage score. This is why the diagnostic gate requires qualitative analysis of the student's actual responses rather than a score threshold.
- Insight 5: **Component skill instruction requires a synthesis step.** *(Added v1.4)* Teaching a student to deconstruct paragraphs, create SPOs, draft from outlines, and elaborate does not guarantee they can do all of these things together without scaffolding. Every multi-lesson paragraph scaffold must conclude with an independent writing task (Write a Free-Form Paragraph for G6-8, Write a Paragraph from Prompt for G3-5) that requires the student to synthesize all component skills autonomously. Without this step, the plan creates an untested assumption of transfer — the same error that Bjork's desirable difficulty framework warns against when learners perform well in guided practice but fail on independent assessment.
...
- **Bjork's Learning vs. Performance Distinction**
   ...
   - Link: Bjork, "Making things hard on yourself, but in a good way" (2011)
   - Insights: The 80% threshold is an operationalization of Bjork's principle, not an empirically derived cutoff from his research. Bjork establishes that desirable difficulties require prior competence but does not specify the percentage where that boundary falls. The actual boundary likely varies by student and skill area. We use 80% because the G6-8 scoring architecture makes it a natural dividing line: a student above 80% has necessarily scored well on the essay (66.7% weight), confirming production-level competence under test conditions. The threshold is justified by the test architecture, validated by Bjork's principle, but should not be cited as if Bjork's research specifies "80%" as a magic number. The system is self-correcting: students near the boundary who are misclassified will retest and receive adjusted plans. *(Updated v1.4)*
